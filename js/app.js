...
var task = {
	...
	initGiftBuy: function () {
		$('.select-gift-price').on('change', function () {
			...
			if (obj.val()) {
				message.text(obj.val() + ' рублей на подарочной карте');
			} else {
				message.text('');
			}
		});
		...
		$('.gift-buy').on('click', function () {
			...
			if (!select.val()) {
				...
				modal.find('.modal-title').text('Вы не выбрали сертификат');
				modal.find('.modal-body').text('Вы не выбрали сколько хотите потратить марок на сертификат');
				modal.modal('show');
				return false;
			}
	...
	initSpecialCreatePage: function () {
		var modal = new BootstrapDialog({
			title: 'Не указан период действия акции',
			closable: true,
		...
		$imageInput.replaceWith($imageInput.clone().on('change', function () {
			if (this.files && this.files[0]) {
				...
				if (fileName.indexOf(".") == -1 || fileExtension != 'jpg' && fileExtension != 'jpeg' && fileExtension != 'png' && fileExtension != 'gif') {
					task.hideLoading();
					alert('Необходимо загружать изображение с разрешением: jpg, png');
					//pr.showErrorModal('Необходимо загружать изображение с разрешением: jpg, png');
					return false;
				}
					...
					$image.cropper({
						...
						ready: function () {
							...
							if (imageData.naturalWidth < needWidth) {
								$errorBlock.text('Ширина изображения должна быть не менее ' + needWidth + 'px');
								$imageBtn.prop('disabled', 'disabled');
							} else if (imageData.naturalHeight < needHeight) {
								$errorBlock.text('Высота изображения должна быть не менее ' + needHeight + 'px');
								$imageBtn.prop('disabled', 'disabled');
							} else {
	...
	showRemaining: function (clock, intervalContainer) {
		...
		clock.text(days + ' ' + declOfNum(days, ['день', 'дня', 'дней']) + ' ' + hours + ":" + minutes + ":" + seconds);
	}
}
